...
/* The data to be sent via USB CDC is stored in this buffer */
...
/* USB virtual serial port default parameters   */
...
VOID USBD_CDC_ACM_Activate(VOID *cdc_acm_instance)
{
  /* USER CODE BEGIN USBD_CDC_ACM_Activate */
    ...
    /* Set USB Virtual Serial Port (UART) baud rate: UART1 as the object */
    CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_baudrate = uart_handler->Init.BaudRate;

    /* Set USB Virtual Serial Port (UART) data type: only 8 and 9 bits are supported */
    switch (uart_handler->Init.WordLength)
    {
        case UART_WORDLENGTH_8B:
        {
        	/* Set USB Virtual Serial Port (UART) data type: 8 bits */
        ...
        {
        	/* Set USB Virtual Serial Port (UART) data type: 9 bits */
    ...
    /* UART1 receives a large amount of data in interrupt mode */
...
VOID USBD_CDC_ACM_ParameterChange(VOID *cdc_acm_instance)
{
  ...
  switch (request)
  {
    case UX_SLAVE_CLASS_CDC_ACM_SET_LINE_CODING :
      ...
      /* Check if the baud rate is less than 115200 and set it to 115200 */
...
VOID usbx_cdc_acm_read_thread_entry(ULONG thread_input)
{
  ...
  while (1)
  {
    ...
    if ((device->ux_slave_device_state == UX_DEVICE_CONFIGURED) && (cdc_acm != UX_NULL))
    {
      ...
      /* The received data is read in blocking mode */
      ux_device_class_cdc_acm_read(cdc_acm, (UCHAR *)UserRxBufferFS, 64,
                                   &actual_length);
      if (actual_length != 0)
      {
    	  /* The data received from USB virtual serial port is sent to UART1 */
